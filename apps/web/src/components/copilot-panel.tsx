...
export function CopilotPanel({
  ...
  const assist = useMutation({
    mutationFn: (body: Record<string, unknown>) => post<Suggestion>('/copilot/assist', { conversationId, ...body }),
    onSuccess: setResult,
  });
  ...
  const actions = [
    ...
    { label: 'Warmer tone', body: { action: 'adjust_tone', tone: 'empathetic', draft }, needsDraft: true },
    { label: 'Translate', body: { action: 'translate', targetLocale: 'Arabic', draft }, needsDraft: true },
  ];

  return (
    <section className="shrink-0 border-t border-border bg-surface-raised px-3 py-2.5" aria-label="AI copilot">
      <div className="mb-2 flex items-center justify-between">
        ...
        <button onClick={onClose} className="text-xs text-text-muted hover:text-text" aria-label="Hide copilot">
          Hide
        </button>
      </div>
      ...
      {assist.isError ? <p className="mt-2 text-xs text-danger">The copilot could not respond. Try again.</p> : null}
            ...
            <>
              ...
              <div className="mt-2 flex flex-wrap items-center gap-1.5">
                ...
                {result.groundedness !== undefined ? (
                  <Badge tone={result.groundedness >= 0.7 ? 'success' : result.groundedness >= 0.4 ? 'warning' : 'danger'}>
          ...
          {result.recommendation ? (
            <div>
              <p className="text-sm font-medium">
                ...
                <Badge tone={result.recommendation.urgency === 'high' ? 'danger' : 'muted'}>{result.recommendation.urgency}</Badge>
              </p>
