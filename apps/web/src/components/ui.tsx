...
export function Badge({ children, tone = 'muted', icon }: { children: ReactNode; tone?: Tone | string; icon?: string }) {
  const resolved = (toneClasses[tone as Tone] ?? toneClasses.muted);
  return (
    <span className={`inline-flex items-center gap-1 rounded-sm border px-1.5 py-0.5 text-xs font-medium ${resolved}`}>
...
export function Button({
  ...
  const variants = {
    ...
    ghost: 'bg-transparent text-text-muted border-transparent hover:bg-surface-sunken hover:text-text',
...
export function Stat({ label, value, sub, tone }: { label: string; value: ReactNode; sub?: string; tone?: Tone }) {
  return (
    <div className="rounded-lg border border-border bg-surface p-4">
      ...
      <p className={`mt-1 text-2xl font-semibold tabular-nums ${tone ? `text-${tone}` : ''}`}>{value}</p>
...
export function Avatar({ name, url, size = 32 }: { name: string | null; url?: string | null; size?: number }) {
  ...
  return url ? (
    ...
    <img src={url} alt={name ?? ''} width={size} height={size} className="rounded-full object-cover" />
  ) : (
    <span
      aria-hidden="true"
...
export function Spinner({ label = 'Loading' }: { label?: string }) {
  return (
    <div className="flex items-center justify-center gap-2 px-6 py-10 text-text-muted" role="status" aria-live="polite">
